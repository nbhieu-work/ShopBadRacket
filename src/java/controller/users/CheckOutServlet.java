...
public class CheckOutServlet extends HttpServlet {
    ...
    private void purchase(HttpServletRequest request, HttpServletResponse response) {
        ...
        List<Products> list = (List<Products>) session.getAttribute("listProduct");
        //check số lượng sản phẩm giỏ hàng so với số lượng sản phẩm trong kho
        for (OrderDetails od : cart.getListOrderDetails()) {
            int quantity = od.getQuantity();
            int productId = od.getProductId();
            int quantityInStock = findQuantityById(list, productId);
        ...
        //update lai so luong san pham trong kho
        for (OrderDetails od : cart.getListOrderDetails()) {
            int quantity = od.getQuantity();
            int productId = od.getProductId();
            int quantityInStock = findQuantityById(list, productId);
        ...
        session.setAttribute("purchaseSuccess", "Mua hàng thành công!");
        //xoa cart tren session
    ...
    private int findQuantityById(List<Products> list, int productId) {
        for (Products product : list) {
